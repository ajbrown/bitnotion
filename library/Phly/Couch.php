...
class Phly_Couch
{
    ...
    /**
     * Return all documents for a view
     *
     * @param	string $designdoc
     * @param	string $view the view function to call
     * @param	array $options query options
     * @return 	Phly_Couch_DocumentSet
     */
    public function openView( $designdoc, $view, array $options = null )
    {
        $db = null;
        if (is_array($options) && array_key_exists('db', $options)) {
            $db = $options['db'];
        ...
        $path =  '_view/' . urlencode( $designdoc ) . '/' . urlencode( $view );
        $response = $this->_prepareAndSend($db . '/' . $path, 'GET', $options);
    ...
    public function docSave($document, $id = null, $db = null)
    {
        ...
        //XXX for some reason, raw data was nno longer set once we entered
        // _prepareAndSend
     ...
     * @param  string $method
     * @param  null|array $queryParams
     * @param  null|string $data raw data to add
     * @return Zend_Http_Response
     */
    protected function _prepareAndSend($path, $method, array $queryParams = null, $data = null)
    {
